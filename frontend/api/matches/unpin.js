...
const { getFirestore, doc, getDoc, updateDoc, arrayRemove, serverTimestamp } = require('firebase/firestore');
...
module.exports = async (req, res) => {
  ...
  // Handle PUT request for unpinning a match
  if (req.method === 'PUT') {
    try {
      ...
      if (!matchData.users.includes(uid)) {
        return res.status(403).json({ message: 'Not authorized to unpin this match' });
      }
      
      // Get feedback from request body
      const { feedback } = req.body || {};
      
      // Update match status to 'ended' and remove user from pinnedBy array
